...
uint32_t Player::getAttackSpeed() const
{
	...
	ret = ret - (ret * getCharacterStat(CHARSTAT_DEXTERITY) / 200.0);
	return ret;
}
                            ...
                            const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (party) {
		...
		if (partyStepsCounter++ > 10)
		{
			partyStepsCounter = 0;
			party->updateMembersPosition(this, false);
		}
	}
...
void Player::onSharedExperience(uint64_t& gainExp)
{
	...
	for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; ++i) {
		...
		int32_t value = item->getAttributeValue(TOOLTIP_ATTRIBUTE_EXPERIENCE);
		if (value != 0) {
			value += value;
		}
	}
