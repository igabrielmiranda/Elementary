...
{
	if (mana != 0) {
		uint32_t finalMana = mana;
		if (Guild* guild = player->getGuild()) {
			if (guild->hasBuff(SUPPORTIVE_3, MANA_COST)) {
				finalMana -= std::round(mana * (GUILD_BUFF_MANA_COST / 100.));
			}
		}
		return finalMana;
	}
		...
		if (hasParam) {
			...
			if (!target || target->getHealth() <= 0) {
				if (!casterTargetOrDirection) {
					if (cooldown > 0) {
						uint32_t finalCooldown = cooldown;
				if (Guild* guild = player->getGuild()) {
					if (guild->hasBuff(SUPPORTIVE_3, COOLDOWN_REDUCTION)) {
						finalCooldown -= std::round(cooldown * (GUILD_BUFF_COOLDOWN / 100.));
					}
				}
				Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, finalCooldown, 0, false, spellId);
						player->addCondition(condition);
