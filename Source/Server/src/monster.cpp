...
{
    // Verifica que el contenedor de botín no sea nulo y que el monstruo realmente deja botín
    if (corpse && lootDrop) {
        // Notifica a los eventos que el monstruo está dejando botín
        g_events->eventMonsterOnDropLoot(this, corpse);

        // Establece el nivel de rareza de los objetos dentro del contenedor
        ...
        // Hace que el jugador haga autoloot de todos los ítems recolectados
...
void Monster::incrementDamage(CombatType_t combatType, int32_t& damage)
{
	const double multiplier = 1 + (g_game.getDailyMonster(getName(), MONSTER_MULTIPLIER_DAMAGE) / 100.);
	if (multiplier != 1.) {
		damage = std::round(damage * multiplier);
	}
}

void Monster::updateMaxHealth()
{
	const double multiplier = 1 + (g_game.getDailyMonster(getName(), MONSTER_MULTIPLIER_HEALTH) / 100.);
	if (multiplier != 1.) {
		healthMax = std::round(healthMax * multiplier);
		health = healthMax;
...
{
	uint64_t experience = skillLoss ? mType->info.experience : 0;
	const double multiplier = 1 + (g_game.getDailyMonster(getName(), MONSTER_MULTIPLIER_EXPERIENCE) / 100.);
	if (multiplier != 1.) {
		experience = std::round(experience * multiplier);
	}
