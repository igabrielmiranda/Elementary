...
void Game::internalUseItem(Player* player, Item* item)
{
	g_actions->useItem(player, item);
}
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field, bool ignoreResistances /*= false */)
{
	...
	if (damage.primary.value > 0) {
		if (attacker) {
			attacker->incrementDamage(damage.primary.type, damage.primary.value);
		}
		return false;
	}
...
void Game::resetDailyMonster()
{
	AvailableMonsterMap& monsters = g_monsters.getAvailableMonsters();

	m_dailyMonsterList.clear();
	std::vector<size_t> dailyMonsters;

	while (m_dailyMonsterList.size() < g_config.getNumber(ConfigManager::DAILY_MONSTERS_QUANTITY)) {
		std::map<MonsterMultipliers_t, uint16_t> multipliers;
		multipliers.emplace(MONSTER_MULTIPLIER_DAMAGE, normal_random(5, 15));
		multipliers.emplace(MONSTER_MULTIPLIER_HEALTH, normal_random(5, 15));
		multipliers.emplace(MONSTER_MULTIPLIER_EXPERIENCE, normal_random(25, 50));
		multipliers.emplace(MONSTER_MULTIPLIER_LOOT, normal_random(35, 75));
		multipliers.emplace(MONSTER_MULTIPLIER_SPAWN, normal_random(75, 125));

		size_t index = uniform_random(0, monsters.size() - 1);
		while (std::find(dailyMonsters.begin(), dailyMonsters.end(), index) != dailyMonsters.end()) {
			index = uniform_random(0, monsters.size() - 1);
		}

		m_dailyMonsterList.emplace(monsters.at(index)->name, multipliers);
		dailyMonsters.push_back(index);
	}

	saveDailyMonsters();
}

void Game::saveDailyMonsters()
{
	DBTransaction transaction;
	Database& db = Database::getInstance();
	if (!transaction.begin()) {
	...
	if (!db.executeQuery("DELETE FROM `daily_monsters`")) {
		return;
	}

	for (const auto& [name, multipliers] : m_dailyMonsterList) {
		DBInsert dailyMonstersQuery("INSERT INTO `daily_monsters` (`name`, `multiplier_damage`, `multiplier_health`, `multiplier_experience`, `multiplier_loot`, `multiplier_spawn`) VALUES");
		if (!dailyMonstersQuery.addRow(fmt::format("{:s}, {:d}, {:d}, {:d}, {:d}, {:d}",
			db.escapeString(name),
			multipliers.at(MONSTER_MULTIPLIER_DAMAGE),
			multipliers.at(MONSTER_MULTIPLIER_HEALTH),
			multipliers.at(MONSTER_MULTIPLIER_EXPERIENCE),
			multipliers.at(MONSTER_MULTIPLIER_LOOT),
			multipliers.at(MONSTER_MULTIPLIER_SPAWN)))) {
			return;
		}

		if (!dailyMonstersQuery.execute()) {
			return;
		}
	}
...
void Game::addDailyMonster(const std::string& name, const std::map<MonsterMultipliers_t, uint16_t>& list)
{
	m_dailyMonsterList.insert_or_assign(name, list);
}

uint16_t Game::getDailyMonster(const std::string& name, MonsterMultipliers_t id) const
{
	const auto itDailyMonster = m_dailyMonsterList.find(name);
	if (itDailyMonster == m_dailyMonsterList.end()) {
		return 0;
	}

	auto itMultiplier = itDailyMonster->second.find(id);
	if (itMultiplier == itDailyMonster->second.end()) {
		return 0;
	}

	return itMultiplier->second;
}
