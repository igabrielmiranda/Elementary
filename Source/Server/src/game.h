...
typedef std::map<std::string, std::map<MonsterMultipliers_t, uint16_t>> DailyMonsterList;
...
class Game
{
	public:
		...
		void internalUseItem(Player* player, Item* item);
		...
		void resetDailyMonster();
		void saveDailyMonsters();
		void addDailyMonster(const std::string& name, const std::map<MonsterMultipliers_t, uint16_t>& list);
		uint16_t getDailyMonster(const std::string& name, MonsterMultipliers_t id) const;
		const DailyMonsterList& getDailyMonsterList() const { return m_dailyMonsterList; }
	...
	private:
		...
		DailyMonsterList m_dailyMonsterList;
