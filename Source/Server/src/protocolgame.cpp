...
void ProtocolGame::parseRemoveLootCategory(NetworkMessage& msg)
{
	...
	addGameTask(&Game::playerRemoveLootCategory, player->getID(), pos, spriteId, stackpos);
}
...
void ProtocolGame::parseAddLootCategory(NetworkMessage& msg)
{
	...
	addGameTask(&Game::playerAddLootCategory, player->getID(), pos, spriteId, stackpos, categoryFlags);
}
...
void ProtocolGame::sendPartyMemberPosition(const std::string& name, uint16_t& vocationId, const Position& position)
{
	NetworkMessage msg;
	msg.addByte(0x2E);
	msg.addString(name);
