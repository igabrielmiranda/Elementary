...
class Player final : public Creature, public Cylinder
{
	public:
		...
		int32_t getMaxHealth() const override {
			uint32_t vitHealth = static_cast<uint32_t>(healthMax * (charStats[CHARSTAT_VITALITY] / 200.f));
		...
		uint32_t getMaxMana() const {
			uint32_t sprMana = static_cast<uint32_t>(manaMax * (charStats[CHARSTAT_SPIRIT] / 200.f));
		...
		void sendPartyMemberPosition(const std::string& name, uint16_t vocationId, const Position& position)
		{
			if (client) {
				client->sendPartyMemberPosition(name, vocationId, position);
			}
		}
