...
lastAwareRangeRequest = { width = 0, height = 0 }
...
function show()
  ...
  lastAwareRangeRequest.width = 0
  lastAwareRangeRequest.height = 0
  gameRootPanel:show()
  gameRootPanel:focus()
...
local function shouldStretchGameMap()
  return not g_settings.getBoolean("classicView") and g_settings.getBoolean("stretchGameMap")
end

local function getStretchAwareRangeTarget(visibleDimension)
  local width = math.max(31, visibleDimension.width + 3)
  local height = math.max(21, visibleDimension.height + 3)

  -- Keep non-classic widescreen within a stable viewport budget.
  width = math.min(width, 45)
  height = math.min(height, 27)

  return width, height
end

local function getGameMapHorizontalInsets()
  local leftInset = gameLeftPanels:getWidth() + gameLeftActionPanel:getWidth()
  local rightInset = gameRightPanels:getWidth() + gameRightActionPanel:getWidth()
  return leftInset, rightInset
end

local function normalizeMapAwareRangeSize(value, minimum)
  value = math.max(minimum, math.floor(value))
  if value % 2 == 0 then
    value = value + 1
  end
...
local function requestMapAwareRange(width, height)
  if not g_game.isOnline() or not g_game.getFeature(GameChangeMapAwareRange) then
    return
  end

  width = normalizeMapAwareRangeSize(width, 19)
  height = normalizeMapAwareRangeSize(height, 15)

  if lastAwareRangeRequest.width == width and lastAwareRangeRequest.height == height then
    return
  end

  lastAwareRangeRequest.width = width
  lastAwareRangeRequest.height = height
  g_game.changeMapAwareRange(width, height)
end
...
--  end
  ...
  g_game.setFeature(GameBiggerMapCache, not classic and g_settings.getBoolean("cacheMap"))
...
function updateSize()
  ...
  if not classic then
    ...
    if shouldStretchGameMap() then
      local awareWidth, awareHeight = getStretchAwareRangeTarget(dimenstion)
      requestMapAwareRange(awareWidth, awareHeight)
      gameMapPanel:setMarginLeft(0)
      gameMapPanel:setMarginRight(0)
    else
      -- Legacy fixed-width mode keeps the map centered inside the playable area.
      local availableWidth = math.max(0, width)
