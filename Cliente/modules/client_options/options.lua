...
function setOption(key, value, force)
  ...
  elseif key == 'displayPartyMembers' then
    modules.game_minimap.displayPartyMembers(value)
  end

  if key == 'classicView' and interfacePanel then
    local cacheMapWidget = interfacePanel:recursiveGetChildById('cacheMap')
    local stretchGameMapWidget = interfacePanel:recursiveGetChildById('stretchGameMap')
    if cacheMapWidget then
      cacheMapWidget:setEnabled(not value)
    end
    if stretchGameMapWidget then
      stretchGameMapWidget:setEnabled(not value)
    end
  end
