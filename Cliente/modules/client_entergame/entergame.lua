...
function dailyMonsterRequest()
  ...
  local server_params = serverHostTextEdit:getText():split(":")
  g_game.setClientVersion(server_params[3])
  g_game.setProtocolVersion(g_game.getClientProtocolVersion(server_params[3]))
  g_game.setCustomProtocolVersion(0)
  g_game.setCustomOs(-1) -- disable
  g_game.chooseRsa(server_params[1])
  ...
  if not modules.game_things.isLoaded() then
	...
	protocolLogin = ProtocolLogin.create()
	protocolLogin.onDailyMonsterRequest = onDailyMonsterRequest
	protocolLogin:dailyMonster(server_params[1], server_params[2])
end

function onDailyMonsterRequest(protocol, dailyMonsters)
	if g_game.isOnline() then
		return true
	end

  modules.client_background.updateDailyMonsters(dailyMonsters)
end
