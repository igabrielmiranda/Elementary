...
local function isPlayerOnPosition(player, pos)
    local tile = Tile(pos)
    if not tile then
        return false
    end
    local topCreature = tile:getTopCreature()
    return topCreature and topCreature == player
end
...
function soulPit.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    for index, pos in pairs(config.participantsPos) do
        if isPlayerOnPosition(player, pos) then
            participants[#participants + 1] = player
        end
    end

    if #participants == 0 then
        player:sendTextMessage(MESSAGE_INFO_DESCR, "You must be standing on one of the designated positions to start the soulpit.")
        return true
    end
