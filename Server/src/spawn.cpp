...
void Spawns::startup()
{
	...
	for (Spawn& spawn : spawnList) {
		for (auto& [spawnId, spawnData] : spawn.spawnMap) {
			const double multiplier = std::max(0., 1 - (g_game.getDailyMonster(spawnData.mTypes.front().first->name, MONSTER_MULTIPLIER_SPAWN) / 100.));
			if (multiplier != 1.) {
				spawnData.interval = std::round(spawnData.interval * multiplier);
			}
		}
		spawn.startup();
	}
