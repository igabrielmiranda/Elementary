...
void MoveEvents::registerEquipItem(const ItemType& itemType)
{
	if (itemType.weaponType != WEAPON_NONE || itemType.ammoType != AMMO_NONE || itemType.attack != 0 ||
	    itemType.defense != 0 || itemType.extraDefense != 0 || itemType.armor != 0 ||
	    itemType.slotPosition & SLOTP_NECKLACE || itemType.slotPosition & SLOTP_RING ||
	    itemType.slotPosition & SLOTP_AMMO || itemType.slotPosition & SLOTP_FEET ||
	    itemType.slotPosition & SLOTP_HEAD || itemType.slotPosition & SLOTP_ARMOR ||
	    itemType.slotPosition & SLOTP_LEGS) {
		static const std::vector<MoveEvent_t> eventList = {MOVE_EVENT_EQUIP, MOVE_EVENT_DEEQUIP};
		for (auto& itEventId : eventList) {
			Event_ptr event = Event_ptr(new MoveEvent(&scriptInterface));
			if (event) {
				MoveEvent_ptr moveEvent{static_cast<MoveEvent*>(event.release())};
				moveEvent->setEventType(itEventId);
				moveEvent->scripted = false;

				if (itEventId == MOVE_EVENT_EQUIP) {
					moveEvent->equipFunction = moveEvent->EquipItem;
				} else {
					moveEvent->equipFunction = moveEvent->DeEquipItem;
				}

				if (itemType.slotPosition & SLOTP_NECKLACE) {
				...
				} else if (itemType.slotPosition & SLOTP_HEAD) {
				...
				} else if (itemType.weaponType != WEAPON_NONE) {
					moveEvent->setSlot(SLOTP_RIGHT | SLOTP_LEFT);
				} else {
					return;
				}

				ItemType& it = Item::items.getItemType(itemType.id);
				it.wieldInfo = moveEvent->getWieldInfo();
				it.minReqLevel = moveEvent->getReqLevel();
				it.minReqMagicLevel = moveEvent->getReqMagLv();
				it.vocationString = moveEvent->getVocationString();

				addEvent(std::move(*moveEvent), itemType.id, itemIdMap);
			}
		}
	}
}
