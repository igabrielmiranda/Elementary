...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			} else if (strcasecmp(attrName, "ignoreDailyMonster") == 0) {
				mType->info.ignoreDailyMonster = attr.as_bool();
			} else {
...
AvailableMonsterMap& Monsters::getAvailableMonsters()
{
	static AvailableMonsterMap m_availableMonsters;
	if (!m_availableMonsters.empty()) {
		// The map is already populated with available monsters
		return m_availableMonsters;
	}

	for (const auto& [name, mType] : monsters) {
		if (mType.info.experience == 0) {
			// Ignore all monsters that give 0 experience
			continue;
		}
		...
		if (mType.info.ignoreDailyMonster) {
			// Ignore monsters with the flag
			continue;
		}

		m_availableMonsters.push_back(&mType);
	}
