	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				if (Guild* guild = casterPlayer->getGuild()) {
					if (targetPlayer) {
						if (guild->hasBuff(OFFENSIVE_3, PLAYER_DAMAGE)) {
							damage.primary.value += std::round(damage.primary.value * (GUILD_BUFF_PLAYER_DAMAGE / 100.));
							damage.secondary.value += std::round(damage.secondary.value * (GUILD_BUFF_PLAYER_DAMAGE / 100.));
						}
					}
					else if (target->getMonster()) {
						if (guild->hasBuff(OFFENSIVE_3, MONSTER_DAMAGE)) {
							damage.primary.value += std::round(damage.primary.value * (GUILD_BUFF_MONSTER_DAMAGE / 100.));
							damage.secondary.value += std::round(damage.secondary.value * (GUILD_BUFF_MONSTER_DAMAGE / 100.));
						}
					}
				}
			}
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE) + casterPlayer->getItemAttributeValue(TOOLTIP_ATTRIBUTE_CRITICALHIT_CHANCE);
	...
	if (success) {
		...
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
			...
			if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
				...
				uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT) + casterPlayer->getItemAttributeValue(TOOLTIP_ATTRIBUTE_LIFE_LEECH_AMOUNT);
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
			...
			if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
				...
				uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT) + casterPlayer->getItemAttributeValue(TOOLTIP_ATTRIBUTE_MANA_LEECH_AMOUNT);
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
					leechCombat.primary.value = std::round(totalDamage * (skill / 100.));
			...
			if (casterPlayer) {
				if (Guild* guild = casterPlayer->getGuild()) {
					if (creature->getPlayer()) {
						if (guild->hasBuff(OFFENSIVE_3, PLAYER_DAMAGE)) {
							damageCopy.primary.value += std::round(damageCopy.primary.value * (GUILD_BUFF_PLAYER_DAMAGE / 100.));
							damageCopy.secondary.value += std::round(damageCopy.secondary.value * (GUILD_BUFF_PLAYER_DAMAGE / 100.));
						}
					}
					else if (creature->getMonster()) {
						if (guild->hasBuff(OFFENSIVE_3, MONSTER_DAMAGE)) {
							damageCopy.primary.value += std::round(damageCopy.primary.value * (GUILD_BUFF_MONSTER_DAMAGE / 100.));
							damageCopy.secondary.value += std::round(damageCopy.secondary.value * (GUILD_BUFF_MONSTER_DAMAGE / 100.));
						}
					}
				}
			}
