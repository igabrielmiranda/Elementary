...
{
	const std::string host = g_config.getString(ConfigManager::MYSQL_HOST);
	const std::string user = g_config.getString(ConfigManager::MYSQL_USER);
	const std::string password = g_config.getString(ConfigManager::MYSQL_PASS);
	const std::string database = g_config.getString(ConfigManager::MYSQL_DB);
	const std::string socket = g_config.getString(ConfigManager::MYSQL_SOCK);
	const auto port = g_config.getNumber(ConfigManager::SQL_PORT);
	...
	// Local XAMPP/MySQL setups usually don't expose SSL, so don't enforce it on loopback.
	if (host == "127.0.0.1" || host == "localhost" || host == "::1") {
		my_bool enforceSsl = 0;
		my_bool verifyServerCert = 0;
		mysql_options(handle, MYSQL_OPT_SSL_ENFORCE, &enforceSsl);
		mysql_options(handle, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, &verifyServerCert);
	}
